...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minValue(state, agentIndex, depth):
            ...
            if not legalActions:  # кінець
                return self.evaluationFunction(state)

            # привиди закінчили хід -> пакман рухається
            if agentIndex == state.getNumAgents() - 1:
                return min(maxValue(state.generateSuccessor(agentIndex, action), depth) for action in legalActions) # пакман
            else:
                return min(minValue(state.generateSuccessor(agentIndex, action), agentIndex + 1, depth) for action in # привиди
                           legalActions)
    ...
    def getAction(self, gameState):
        ...
        def minValue(state, agentIndex, depth, a, b):
            ...
            for action in legalActions:
                ...
                if agentIndex == state.getNumAgents() - 1: # останній привид?
